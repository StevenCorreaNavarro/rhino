...
def get_adjustments_summary(start_dt, end_dt):
    """
    Devuelve lista de ajustes entre fechas:
    cada fila tiene id, kind, note, amount, user, created_at
    """
    c = conn.cursor()
    c.execute("""
        SELECT id, kind, note, amount, user, created_at
        FROM adjustments
        WHERE created_at BETWEEN ? AND ?
...
# 
        ...
        ttk.Button(navbar, text="💰 Cierre de caja", command=self.open_cash_closure_window).pack(side=tk.LEFT, padx=6, pady=4)
    ...
    def open_window_once(self, key, creator):
        ...
        win = creator()
        # VALIDACIÓN: el creator debe devolver un widget Tk (Toplevel/Frame/etc.)
        if not hasattr(win, 'winfo_exists'):
            raise RuntimeError(f"Creator for window key '{key}' did NOT return a Tk widget. Returned: {type(win).__name__} -> {repr(win)}")
        ...
        try:
            ...
            # algunos objetos no implementan protocol; ignore
            pass
        return win
    ...
    def open_cash_closure_window(self):
        """
        Ventana interactiva de cierre de caja:
        - seleccionar periodo (desde/hasta)
        - calcular resumen: productos vendidos, formas de pago, créditos, deudas, ajustes
        - exportar CSV
        - registrar cierre (usa save_cash_closure)
        """
        def creator():
            win = tk.Toplevel(self.root)
            ...
            # Asegurar que la ventana quede al frente y con foco
            win.lift()
            win.focus_force()
            try:
                win.grab_set()
            ...
            # --- Top: rango de fechas y botón calcular ---
            top = ttk.Frame(win, padding=8); top.pack(fill=tk.X)
            ttk.Label(top, text="Desde (YYYY-MM-DD HH:MM:SS):").pack(side=tk.LEFT)
            from_var = tk.StringVar(value=(datetime.now().replace(hour=0, minute=0, second=0)).strftime("%Y-%m-%d %H:%M:%S"))
            from_entry = ttk.Entry(top, textvariable=from_var, width=22); from_entry.pack(side=tk.LEFT, padx=6)
            ttk.Label(top, text="Hasta (YYYY-MM-DD HH:MM:SS):").pack(side=tk.LEFT, padx=(12,0))
            to_var = tk.StringVar(value=datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
            to_entry = ttk.Entry(top, textvariable=to_var, width=22); to_entry.pack(side=tk.LEFT, padx=6)
            ttk.Button(top, text="Calcular resumen", command=lambda: do_calculate()).pack(side=tk.LEFT, padx=8)
            ttk.Button(top, text="Exportar todo (CSV)", command=lambda: export_all_csv()).pack(side=tk.RIGHT, padx=6)
    
            # --- Middle: left (productos + pagos) and right (resumen y acciones) ---
            ...
            right = ttk.Frame(middle, width=340); right.pack(side=tk.LEFT, fill=tk.Y)
    
            # Productos vendidos (resumen)
            ttk.Label(left, text="Productos vendidos (resumen)", font=(None, 11, "bold")).pack(anchor=tk.W)
            prod_frame = ttk.Frame(left); prod_frame.pack(fill=tk.BOTH, expand=True, pady=(4,10))
            prod_tree = ttk.Treeview(prod_frame, columns=('code','name','qty','subtotal'), show='headings', height=14)
            prod_tree.heading('code', text='Código'); prod_tree.heading('name', text='Producto')
            prod_tree.heading('qty', text='Cant.'); prod_tree.heading('subtotal', text='Subtotal')
            prod_tree.column('code', width=100, anchor=tk.W)
            prod_tree.column('name', width=320, anchor=tk.W)
            prod_tree.column('qty', width=80, anchor=tk.E)
            prod_tree.column('subtotal', width=120, anchor=tk.E)





            vsb = ttk.Scrollbar(prod_frame, orient='vertical', command=prod_tree.yview)
            prod_tree.configure(yscroll=vsb.set)
            vsb.pack(side=tk.RIGHT, fill=tk.Y)
            prod_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)



            # después de crear prod_tree (Productos vendidos), añade:
            ttk.Label(left, text="Salidas / Gastos (detalle)", font=(None, 11, "bold")).pack(anchor=tk.W, pady=(8,0))
            out_frame = ttk.Frame(left); out_frame.pack(fill=tk.BOTH, expand=False, pady=(4,8))
            out_tree = ttk.Treeview(out_frame, columns=('id','when','amount','desc','user'), show='headings', height=6)
            out_tree.heading('id', text='ID'); out_tree.heading('when', text='Fecha'); out_tree.heading('amount', text='Monto')
            out_tree.heading('desc', text='Descripción'); out_tree.heading('user', text='Usuario')
            out_tree.column('id', width=50, anchor=tk.CENTER)
            out_tree.column('when', width=140)
            out_tree.column('amount', width=110, anchor=tk.E)
            out_tree.column('desc', width=260)
            out_tree.column('user', width=80, anchor=tk.CENTER)
            out_tree.pack(fill=tk.BOTH, expand=True)
            
    
            # Pagos por método (resumen)
            ttk.Label(left, text="Formas de pago (resumen)", font=(None, 11, "bold")).pack(anchor=tk.W, pady=(8,0))
            pay_tree = ttk.Treeview(left, columns=('method','total','sales'), show='headings', height=6)
            pay_tree.heading('method', text='Método'); pay_tree.heading('total', text='Total'); pay_tree.heading('sales', text='Ventas')
            pay_tree.column('method', width=180)
            pay_tree.column('total', width=140, anchor=tk.E)
            pay_tree.column('sales', width=80, anchor=tk.CENTER)
            pay_tree.pack(fill=tk.X, pady=(4,8))
    
            # Right side: resumen estadístico y acciones
            ttk.Label(right, text="Resumen & Acciones", font=(None, 11, "bold")).pack(anchor=tk.W)
            summary_box = ttk.Frame(right); summary_box.pack(fill=tk.X, pady=(6,6))
            total_sales_var = tk.StringVar(value="0")
            ttk.Label(summary_box, text="Total ventas (monto):").grid(row=0, column=0, sticky=tk.W)
            ttk.Label(summary_box, textvariable=total_sales_var).grid(row=0, column=1, sticky=tk.E)
    
            ttk.Label(summary_box, text="Efectivo en ventas:").grid(row=1, column=0, sticky=tk.W, pady=(6,0))
            cash_sales_var = tk.StringVar(value="0"); ttk.Label(summary_box, textvariable=cash_sales_var).grid(row=1, column=1, sticky=tk.E, pady=(6,0))
    
            ttk.Label(summary_box, text="Créditos creados:").grid(row=2, column=0, sticky=tk.W, pady=(6,0))
            credits_var = tk.StringVar(value="0"); ttk.Label(summary_box, textvariable=credits_var).grid(row=2, column=1, sticky=tk.E, pady=(6,0))
    
            ttk.Label(summary_box, text="Deudas creadas:").grid(row=3, column=0, sticky=tk.W, pady=(6,0))
            debts_var = tk.StringVar(value="0"); ttk.Label(summary_box, textvariable=debts_var).grid(row=3, column=1, sticky=tk.E, pady=(6,0))
    
            ttk.Label(summary_box, text="Ajustes (total):").grid(row=4, column=0, sticky=tk.W, pady=(6,0))
            adjusts_var = tk.StringVar(value="0"); ttk.Label(summary_box, textvariable=adjusts_var).grid(row=4, column=1, sticky=tk.E, pady=(6,0))
    
            # Inputs para registro de cierre
            ttk.Separator(right, orient='horizontal').pack(fill=tk.X, pady=8)
            ttk.Label(right, text="Registrar cierre", font=(None, 10, "bold")).pack(anchor=tk.W)
            rc_frame = ttk.Frame(right); rc_frame.pack(fill=tk.X, pady=(6,6))
            ttk.Label(rc_frame, text="Apertura caja:").grid(row=0, column=0, sticky=tk.W)
            opening_var = tk.StringVar(value="0"); ttk.Entry(rc_frame, textvariable=opening_var, width=18).grid(row=0, column=1, sticky=tk.E)
            ttk.Label(rc_frame, text="Efectivo contado:").grid(row=1, column=0, sticky=tk.W, pady=(6,0))
            counted_var = tk.StringVar(value="0"); ttk.Entry(rc_frame, textvariable=counted_var, width=18).grid(row=1, column=1, sticky=tk.E, pady=(6,0))
            ttk.Label(rc_frame, text="Gastos en efectivo:").grid(row=2, column=0, sticky=tk.W, pady=(6,0))
            expenses_var = tk.StringVar(value="0"); ttk.Entry(rc_frame, textvariable=expenses_var, width=18).grid(row=2, column=1, sticky=tk.E, pady=(6,0))
            ttk.Label(rc_frame, text="Notas:").grid(row=3, column=0, sticky=tk.W, pady=(6,0))
            notes_var = tk.StringVar(); ttk.Entry(rc_frame, textvariable=notes_var, width=28).grid(row=3, column=1, sticky=tk.E, pady=(6,0))
            ...
            ttk.Button(btnf, text="Cerrar", command=win.destroy).pack(fill=tk.X)
    
            # ---- helper functions ----
            def clear_views():
                for t in (prod_tree, pay_tree):
            ...
            def do_calculate():
                ...
                # Productos vendidos (igual que antes)...
                rows = get_sale_items_for_sales(start_dt, end_dt)
                total_sales_amount = 0
                for r in rows:
                    rr = dict(r)
                    code = rr.get('product_code') or ""
                    name = rr.get('product_name') or ""
                    qty = int(rr.get('qty') or 0)
                    subtotal = int(rr.get('subtotal') or 0)
                    prod_tree.insert('', tk.END, values=(code, name, qty, f"${format_money(subtotal)}"))
                    total_sales_amount += subtotal
            
                # Pagos por método (igual que antes)...
                pays = get_payments_summary(start_dt, end_dt)
                payments_summary = {}
                cash_total = 0
                for p in pays:
                    pp = dict(p)
                    method = pp.get('method') or "Desconocido"
                    tot = int(pp.get('total') or 0)
                    cnt = int(pp.get('sales_count') or 0)
                    pay_tree.insert('', tk.END, values=(method, f"${format_money(tot)}", cnt))
                    payments_summary[method] = tot
                    if method.lower() in ("efectivo", "cash"):
                        cash_total += tot
            
                # Ajustes / salidas detalladas
                out_rows = get_adjustments_summary(start_dt, end_dt)
                total_out = 0
                for orow in out_rows:
                    orr = dict(orow)
                    # sólo consideramos salidas de caja (cash_expense)
                    if orr.get('kind') != 'cash_expense':
                        continue
                    oid = orr.get('id')
                    when = orr.get('created_at') or ""
                    amt = int(orr.get('amount') or 0)
                    desc = orr.get('note') or ""
                    user = orr.get('user') or ""
                    out_tree.insert('', tk.END, values=(oid, when, f"${format_money(amt)}", desc, user))
                    total_out += amt
            
                # Actualizar variables de la derecha
                total_sales_var.set(f"${format_money(total_sales_amount)}")
                cash_sales_var.set(f"${format_money(cash_total)}")
                adjusts_var.set(f"${format_money(total_out)}")
            
                # Calcular neto de efectivo: efectivo recaudado menos salidas
                net_cash = cash_total - total_out
                # Efectivo disponible si consideras apertura y otros gastos manuales (expenses_var)
                try:
                    opening = parse_money_to_int(opening_var.get())
                    manual_expenses = parse_money_to_int(expenses_var.get())
                except:
                    opening = 0; manual_expenses = 0
                efectivo_disponible = opening + cash_total - total_out - manual_expenses
            
                # mostrar netos en widgets (añade variables si no existen)
                net_total_var = tk.StringVar(f"${format_money(net_cash)}")
                # si quieres mostrar efectivo disponible:
                # create or update a label:
                if not hasattr(win, "_efectivo_lbl"):
                    win._efectivo_lbl = ttk.Label(right, text=f"Efectivo disponible (apertura+efectivo - salidas - gastos): ${format_money(efectivo_disponible)}")
                    win._efectivo_lbl.pack(anchor=tk.W, pady=(6,0))
                else:
                    win._efectivo_lbl.config(text=f"Efectivo disponible (apertura+efectivo - salidas - gastos): ${format_money(efectivo_disponible)}")
            
                # guardar estado para registrar
                win._closure_state = {
                    "start": start_dt,
                    "end": end_dt,
                    "products_total": total_sales_amount,
                    "payments_summary": payments_summary,
                    "total_outflows": total_out,
                    "cash_total": cash_total,
                    "effective_available": efectivo_disponible
                }
                ...
                try:
                    ...
                    messagebox.showerror("Error", f"No se pudo obtener productos vendidos: {e}"); return
                total_sales_amount = 0
                for r in rows:
                    rr = dict(r)
                    code = rr.get('product_code') or ""
                ...
                try:
                    ...
                    messagebox.showerror("Error", f"No se pudo obtener pagos: {e}"); pays = []
                payments_summary = {}
                cash_total = 0
                for p in pays:
                    pp = dict(p)
                ...
                # créditos, deudas, ajustes
                try:
                    cr = get_credits_summary(start_dt, end_dt) or {}
                except:
                    cr = {}
                try:
                    dbt = get_debts_summary(start_dt, end_dt) or {}
                except:
                    dbt = {}
                try:
                    adj = get_adjustments_summary(start_dt, end_dt) or []
                except:
                    adj = []
    
                # update right summary variables
                total_sales_var.set(f"${format_money(total_sales_amount)}")
                cash_sales_var.set(f"${format_money(cash_total)}")
                credits_var.set(f"{(cr.get('created') if isinstance(cr, dict) else (cr['created'] if cr and 'created' in cr else 0)) or 0} - ${format_money((cr.get('total_created') if isinstance(cr, dict) else (cr['total_created'] if cr and 'total_created' in cr else 0)) or 0)}")
                debts_var.set(f"{(dbt.get('created') if isinstance(dbt, dict) else (dbt['created'] if dbt and 'created' in dbt else 0)) or 0} - ${format_money((dbt.get('total_created') if isinstance(dbt, dict) else (dbt['total_created'] if dbt and 'total_created' in dbt else 0)) or 0)}")
                total_adjusts = 0
                for a in adj:
                    a2 = dict(a)
                    total_adjusts += int(a2.get('total_amount') or 0)
                adjusts_var.set(f"${format_money(total_adjusts)}")
    
                # store state for registration
                win._closure_state = {
                    "start": start_dt,
                    "end": end_dt,
                    "products_total": total_sales_amount,
            ...
            def do_register_closure():
                ...
                try:
                    ...
                    expenses = parse_money_to_int(expenses_var.get())
                except Exception:
                    messagebox.showwarning("Error", "Valores inválidos"); return
                cash_in_sales = st['payments_summary'].get('Efectivo', st['payments_summary'].get('cash', 0))
                total_sales = st['products_total']
                cid = save_cash_closure(
                    user=getattr(self, 'current_user', 'cajero'),
                    opened_at=st['start'],
                    closed_at=st['end'],
                    ...
                    payments_summary=st['payments_summary'],
                    notes=notes_var.get().strip()
                )
                messagebox.showinfo("Cierre registrado", f"Cierre guardado (ID: {cid})")
                win.destroy()
    
            def export_all_csv():
                # exporta productos vendidos y pagos a dos CSVs (pide ruta)
                # from tkinter import filedialog
                from tkinter import filedialog
                import csv
                ...
                # productos
                rows = get_sale_items_for_sales(start_dt, end_dt)
                path1 = filedialog.asksaveasfilename(defaultextension=".csv", title="Guardar productos vendidos como...", initialfile="productos_vendidos.csv", filetypes=[("CSV","*.csv")])
                if path1:
                    with open(path1, "w", newline="", encoding="utf-8") as f:
                        w = csv.writer(f)
                        w.writerow(["product_id","code","name","qty","subtotal"])
                        for r in rows:
                            rr = dict(r)
                            w.writerow([rr.get('product_id'), rr.get('product_code'), rr.get('product_name'), rr.get('qty'), rr.get('subtotal')])
                # pagos
                pays = get_payments_summary(start_dt, end_dt)
                path2 = filedialog.asksaveasfilename(defaultextension=".csv", title="Guardar pagos como...", initialfile="pagos_resumen.csv", filetypes=[("CSV","*.csv")])
                if path2:
                    with open(path2, "w", newline="", encoding="utf-8") as f:
                        w2 = csv.writer(f)
                        w2.writerow(["method","total","sales_count"])
                        for p in pays:
                            pp = dict(p)
                            w2.writerow([pp.get('method'), pp.get('total'), pp.get('sales_count')])
                messagebox.showinfo("Exportado", "Exportados CSV correctamente")
    
            # Bind escape and focus
            win.bind("<Escape>", lambda e: win.destroy())
    ...
    def checkout(self):
        ...
        def creator():
            ...
            def on_enter_quickpay(event=None):
                # si alguna entrada (como un Entry) tiene el foco y quieres evitar quickpay, puedes chequear:
                focused = win.focus_get()
                # si el foco está en details_entry o amount_entry y quieres que Enter añada en vez de finalizar
                # ya estamos enlazando amount_entry -> add_payment_line con <Return>, así que aquí priorizamos
                # finalización rápida solo si NO estamos dentro de amount_entry (para evitar conflicto).
                if focused is amount_entry:
                    # si el cajero está escribiendo monto y presiona Enter, añadimos ese pago y NO finalizamos.
                    add_payment_line()
                    return "break"
